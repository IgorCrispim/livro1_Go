...
func main() {
	...
	mux.HandleFunc("GET /{$}", home) // Now restrict this route to exact matches on / only.
	// Add the two news handlers
	mux.HandleFunc("GET /snippet/view/{id}", snippetView) // Now add the {id} wildcard segment
